...
<main class="planning">
  <aside class="control">
    ...
    <section><div class="title"><h2>灭火任务目标</h2><b>{{ targets.length }} 处</b></div>
      <div class="actions"><button @click="recommendTargets">按最新火线推荐</button><button @click="pick('target')">地图增加目标</button></div>
    ...
    <div v-if="targets.length && teams.length>targets.length" class="spare">另有 {{ teams.length-targets.length }} 支队伍可待命。</div>
    <button class="primary" :disabled="busy||!teams.length||!targets.length" @click="plan">{{ busy?'计算中…':'生成队伍到目标的路径' }}</button>
  ...
  <Teleport defer to="#business-panel"><aside class="results">
    ...
    <article v-for="(item,index) in plans" :key="index"><strong>消防队 {{ item.team_index+1 }} → 目标 {{ item.target_index+1 }}</strong><b>{{ item.route.success ? Number(item.route.distance_km).toFixed(2)+' km · '+Number(item.route.estimated_travel_time_minutes).toFixed(1)+' 分钟' : '不可达' }}</b><small>风险值 {{ item.route.success ? Number(item.route.risk_score).toFixed(2) : '--' }} · 风险感知 A*</small></article>
...
type Plan={team_index:number;target_index:number;route:any}
...
const spreadRunId=computed(()=>incident.spreadRunId || fireEvent.spreadRun?.run_id || '')
const spreadArea=computed(()=>Number(fireEvent.spreadRun?.final_area_km2||0))
const spreadRadius=computed(()=>Number(fireEvent.spreadRun?.max_radius_km||0))
const scenarioAwaiting=computed(()=>incident.workflow?.current_stage==='scenario' && incident.workflow?.status==='WAITING_FOR_INPUT')
const fireFront=computed(()=>incident.spreadRunId && incident.workflow?.artifacts?.fire_front?.features?.length ? incident.workflow.artifacts.fire_front : fireEvent.forefireResult?.geojson)
...
function randomTeam(index:number){const angle=(index*137.5+40+Math.random()*25)*Math.PI/180,radius=.035+Math.random()*.025;return point(center.value[0]+Math.cos(angle)*radius,center.value[1]+Math.sin(angle)*radius)}
function invalidate(){plans.value=[];unassigned.value=[];error.value='';void nextTick(draw)}
...
function latestFireRing():{ring:[number,number][];properties:any}|null{
 const features=(fireFront.value?.features||[]).filter((x:any)=>x.geometry?.type==='Polygon'&&Array.isArray(x.geometry.coordinates?.[0]))
 ...
 const ring=(feature.geometry.coordinates[0]||[]).filter((p:any)=>Array.isArray(p)&&p.length>=2).map((p:any)=>[Number(p[0]),Number(p[1])] as [number,number])
...
function draw(){const map=mapRef.value;if(!map)return;map.clearDemoEntities()
 const front=latestFireRing();if(front)map.addDemoArea({name:'当前推演火线',coordinates:front.ring,color:'#fb923c',label:'推演火线'})
...
async function plan(){busy.value=true;error.value='';try{const c=center.value;const response=await fetch('/api/planning/preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_id:incident.eventId,teams:teams.value.map(({longitude,latitude})=>({longitude,latitude})),targets:targets.value.map(({longitude,latitude})=>({longitude,latitude})),fire_center:{longitude:c[0],latitude:c[1]},fire_radius_km:spreadRadius.value})});const data=await response.json();if(!response.ok)throw new Error(typeof data.detail==='string'?data.detail:'路径计算失败 '+response.status);plans.value=data.plans||[];unassigned.value=data.unassigned_target_indices||[];await nextTick();draw()}catch(e:any){error.value=e?.message||'路径计算失败'}finally{busy.value=false}}
onMounted(async()=>{try{await incident.loadEvents();await incident.loadLatestWorkflow();await fireEvent.loadLatestSpreadRun(incident.eventId)}catch(e:any){error.value=e?.message||'事件或推演结果读取失败'}generateTeams(4);recommendTargets();if(route.query.auto_plan==='1'&&targets.value.length)void plan();await nextTick();mapRef.value?.on('load',()=>{mapRef.value?.flyTo({center:center.value,height:85000});draw()})})
watch(()=>incident.eventId,async()=>{teams.value=[];targets.value=[];plans.value=[];try{await incident.loadLatestWorkflow();await fireEvent.loadLatestSpreadRun(incident.eventId)}catch{}generateTeams(4);recommendTargets()})
watch(()=>incident.spreadRunId,async()=>{if(incident.spreadRunId){await fireEvent.loadLatestSpreadRun(incident.eventId);recommendTargets()}})
</script>
<style scoped>
