...
from app.agents.commander_agent import build_structured_command_summary
from app.agents.context import DecisionContext
from app.agents.orchestrator import MultiAgentOrchestrator
from app.agents.risk_agent import _risk_label as _agent_risk_label
from app.agents.risk_agent import _risk_level as _agent_risk_level
...
def _risk_level(area: float, fwi: float, confidence: float) -> str:
    return _agent_risk_level(area, fwi, confidence)


def _risk_label(level: str) -> str:
    return _agent_risk_label(level)


def _agent_output_summary(output: dict[str, Any]) -> dict[str, Any]:
    if "situation_summary" in output:
        summary = output["situation_summary"]
        return {
            "event": summary.get("event", {}).get("event_id"),
            "confidence": summary.get("trusted_fire_point", {}).get("confidence"),
            "wind_speed_m_s": summary.get("environment", {}).get("wind_speed_m_s"),
        }
    if "spread_summary" in output:
        summary = output["spread_summary"]
        return {
    ...
    if "risk_level" in output:
        return {
            ...
            "risk_factor_count": len(output.get("risk_factors", [])),
        }
    if "recommended_plan" in output:
        plan = output["recommended_plan"]
        return {
            "plan_id": plan.get("plan_id"),
            "score": plan.get("score"),
            "risk_level": plan.get("risk_level"),
        }
    return {key: output[key] for key in list(output)[:3]}
...
def _build_packets(
    *,
    ...
    if not llm_text:
        llm_text = build_structured_command_summary(input_summary=input_summary)
...
async def create_decision_run(db: AsyncSession, event_id: str, request: DecisionRunRequest) -> dict[str, Any]:
    ...
    input_summary, packages, warnings = _build_packets(
        ...
        provider="structured",
        model="structured-command-rules",
        trusted=trusted,
        fusion=fusion,
        env=env,
        spread=spread,
    ...
    raw_llm_output = packages["recommendation_packet"]["summary"]
    try:
        ...
        orchestration = await MultiAgentOrchestrator().run(context, force_provider=request.force_provider)
        agent_results = orchestration["agent_results"]
        analysis_context = orchestration["analysis_context"]
        commander_result = orchestration["commander_result"]
        logger.info("Situation Analysis: %s", analysis_context.get("situation", {}))
        logger.info("Spread Analysis: %s", analysis_context.get("spread", {}))
        logger.info("Risk Analysis: %s", analysis_context.get("risk", {}))
        logger.info("Commander Result: %s", commander_result)
        if commander_result and commander_result.get("status") == "success":
            commander_output = commander_result.get("output", {})
            plan_packet = commander_output.get("plan_packet")
            recommendation_packet = commander_output.get("recommendation_packet")
            if plan_packet and recommendation_packet:
                packages["plan_packet"] = plan_packet
                packages["recommendation_packet"] = recommendation_packet
                llm_info = commander_output.get("llm", {})
                provider_name = llm_info.get("provider", provider_name)
                model_name = llm_info.get("model", model_name)
                raw_llm_output = llm_info.get("raw_output", raw_llm_output)
        for result in agent_results:
            logger.info(
                "Decision agent bypass result: agent=%s status=%s summary=%s",
                result.get("agent_name"),
                result.get("status"),
                _agent_output_summary(result.get("output", {})),
            )
    except Exception:
        agent_results = []
        analysis_context = {}
        commander_result = None
        logger.exception("Decision agent bypass failed; legacy decision packages remain in use.")
    ...
    await websocket_manager.broadcast_event(
        event_id,
        "decision.completed",
        {"decision_run_id": decision_run_id, "provider": provider_name, "model": model_name, "packages": packages},
    )
