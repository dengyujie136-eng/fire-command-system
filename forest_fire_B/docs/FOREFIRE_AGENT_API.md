...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `entry_point` | object | 高风险目标入口点，包含 `longitude` / `latitude` |
| `assembly_point` | object | 南侧安全集结点，包含 `longitude` / `latitude` |
| `route_options` | array | 前端优先读取的三条路线：主疏散、消防救援接近、备用安全路径 |
| `evacuation_routes` | array | 兼容字段，主疏散路线 |
| `rescue_routes` | array | 兼容字段，消防救援接近路线 |
| `blocked_routes` | array | 管制或高风险道路 |
| `risk_zones` | array | 火场和烟羽风险区 |
| `summary` | string | 中文路线规划摘要 |

`route_options[]` 字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | `main_evacuation` / `fire_rescue_approach` / `safe_route` |
| `name` | string | 中文路线名称 |
| `type` | string | `evacuation` / `rescue_approach` / `safe` |
| `priority` | string | `recommended` / `secondary` / `fallback` |
| `risk` | string | `low` / `medium` / `high` |
| `distance_km` | number | 路线距离，单位 km |
| `eta_min` | number | 预计耗时，单位 min |
| `color` | string | 前端绘制颜色 |
| `reason` | string | 中文规划理由 |
| `risk_summary` | string | 中文风险说明 |
| `recommended` | boolean | 是否推荐 |
| `coordinates` | array | `[longitude, latitude]` 坐标数组，按行进顺序排列 |
| `elevation_profile` | array | 高程剖面，`distance_km` 从 0 递增 |
...
```text
ws://localhost:8100/ws/agent/forefire/decision
```
...
```ts
const ws = new WebSocket('ws://localhost:8100/ws/agent/forefire/decision')
